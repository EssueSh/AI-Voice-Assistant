...
def audio_playback_worker(audio_queue) -> tuple[bool, None]:
    ...
    """
    print("Starting audio_playback_worker...")
    global timing_info
    ...
    try:
        while True:
            ...
            print("audio_data_from_audio_queue",'is',audio_data)

            if audio_data is not None:
                print("Audio data received for playback.")

                if not timing_info["first_audio_play"]:
                    timing_info["first_audio_play"] = time.perf_counter()
                    print("Timing info updated with first_audio_play timestamp.")

                print("Starting play_audio_with_interrupt...")
                print("audio_data_for _queue",':',audio_data)
                was_interrupted, interrupt_data = play_audio_with_interrupt(audio_data)
                print(f"Playback result — Interrupted: {was_interrupted}")
            ...
            if (
                ...
                print("Audio queue is no longer running and all queues are empty. Exiting loop.")
                break

    except Exception as e:
        print(f"Error in audio playback: {str(e)}")

    print("Exiting audio_playback_worker...")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if audio_data is not None:
                        print("[DEBUG] Audio captured. Running VAD...")
                        speech_segments = detect_speech_segments(vad_pipeline, audio_data)

                        print(f"[DEBUG] Speech segments detected: {speech_segments}")
                        if speech_segments is not None and len(speech_segments) > 0:
                            ...
                            if user_input.strip():
                                ...
                                if was_interrupted and speech_data is not None:
                                    print("[DEBUG] User interrupted with speech.")
